...
#/project/tools2/bin/procctl 30 /project/tools2/bin/checkproc

# 压缩数据中心后台服务程序的备份日志。
/project/tools2/bin/procctl 300 /project/tools2/bin/gzipfiles /log/idc "*.log.20*" 0.02

# 生成用于测试的全国气象站点观测的分钟数据。
/project/tools2/bin/procctl  60 /project/idc2/bin/crtsurfdata /project/idc2/ini/stcode.ini /tmp/idc/surfdata /log/idc/crtsurfdata.log xml,json,csv

# 清理原始的全国气象站点观测的分钟数据目录/tmp/idc/surfdata中的历史数据文件。
/project/tools2/bin/procctl 300 /project/tools2/bin/deletefiles /tmp/idc/surfdata "*" 0.02

# 采集全国气象站点观测的分钟数据
/project/tools2/bin/procctl 30 /project/tools2/bin/ftpgetfiles /log/idc/ftpgetfiles_surfdata.log "<host>192.168.80.132:21</host><mode>1</mode><username>root</username><password>1932048851</password><localpath>/idcdata/surfdata</localpath><remotepath>/tmp/idc/surfdata</remotepath><matchname>*.XML,*.CSV,*.JSON</matchname><listfilename>/idcdata/ftplist/ftpgetfiles_surfdata.list</listfilename><ptype>1</ptype><remotepathbak>/tmp/idc/surfdatabak</remotepathbak><okfilename>/idcdata/ftplist/ftpgetfiles_surfdata.xml</okfilename><checktime>true</checktime><timeout>80</timeout><pname>ftpgetfiles_surfdata</pname>"

# 上传全国气象站点观测的分钟数据的xml文件。
/project/tools2/bin/procctl 30 /project/tools2/bin/ftpputfiles /log/idc/ftpputfiles_surfdata.log "<host>192.168.80.132:21</host><mode>1</mode><username>root</username><password>1932048851</password><localpath>/tmp/idc/surfdata</localpath><remotepath>/tmp/ftpputest</remotepath><matchname>*.XML,*.CSV,*.JSON</matchname><ptype>1</ptype><okfilename>/idcdata/ftplist/ftpputfiles_surfdata.xml</okfilename><timeout>80</timeout><pname>ftpputfiles_surfdata</pname>"

# 清理采集的全国气象站点观测的分钟数据目录/idcdata/surfdata中的历史数据文件。
/project/tools2/bin/procctl 300 /project/tools2/bin/deletefiles /idcdata/surfdata "*" 0.02

# 清理采集的全国气象站点观测的分钟数据目录/tmp/ftpputest中的历史数据文件。
/project/tools2/bin/procctl 300 /project/tools2/bin/deletefiles /tmp/ftpputest "*" 0.02
...
/project/tools2/bin/procctl 300 /project/tools2/bin/deletefiles /tmp/tcpgetest "*" 0.02

# 把全国站点参数数据保存到数据库表中，如果站点不存在则插入，站点已存在则更新。
/project/tools2/bin/procctl 120 /project/idc2/bin/obtcodetodb /project/idc2/ini/stcode.ini "127.0.0.1,root,123456Aa.,mysql,3306" utf8 /log/idc/obtcodetodb.log

# 把全国站点分钟观测数据保存到数据库的T_ZHOBTMIND表中，数据只插入，不更新。
/project/tools2/bin/procctl 10 /project/idc2/bin/obtmindtodb /idcdata/surfdata "127.0.0.1,root,123456Aa.,mysql,3306" utf8 /log/idc/obtmindtodb.log

# 清理T_ZHOBTMIND表中120分之前的数据，防止磁盘空间被撑满。
# /project/tools2/bin/procctl 120 /project/tools2/bin/execsql /project/idc2/sql/cleardata.sql "127.0.0.1,root,123456Aa.,mysql,3306" utf8 /log/idc/execsql.log
...
/project/tools2/bin/procctl 30 /project/tools2/bin/dminingmysql /log/idc/dminingmysql_ZHOBTMIND_vip2.log "<connstr>127.0.0.1,root,123456Aa.,mysql,3306</connstr><charset>utf8</charset><selectsql>select obtid,date_format(ddatetime,'%%Y-%%m-%%d %%H:%%i:%%s'),t,p,u,wd,wf,r,vis,keyid from T_ZHOBTMIND where keyid>:1 and ddatetime>timestampadd(minute,-65,now())</selectsql><fieldstr>obtid,ddatetime,t,p,u,wd,wf,r,vis,keyid</fieldstr><fieldlen>10,19,8,8,8,8,8,8,8,15</fieldlen><bfilename>ZHOBTMIND</bfilename><efilename>HYCZ</efilename><outpath>/idcdata/xmltodb/vip2</outpath><incfield>keyid</incfield><timeout>30</timeout><pname>dminingmysql_ZHOBTMIND_vip2</pname><maxcount>1000</maxcount><connstr1>127.0.0.1,root,123456Aa.,mysql,3306</connstr1>"

# 清理/idcdata/xmltodb/vip2bak和/idcdata/xmltodb/vip2err目录中文件，防止把空间撑满。
/project/tools2/bin/procctl 300 /project/tools2/bin/deletefiles /idcdata/xmltodb/vip2    "*" 0.02
/project/tools2/bin/procctl 300 /project/tools2/bin/deletefiles /idcdata/xmltodb/vip2bak "*" 0.02
/project/tools2/bin/procctl 300 /project/tools2/bin/deletefiles /idcdata/xmltodb/vip2err "*" 0.02
