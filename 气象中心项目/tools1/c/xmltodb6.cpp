...
{
  char colname[31];      // 列名
  char datatype[31];     // 列的数据结构，分为number, date和char三大类
  int collen;            // 列的长度，number固定位20, date固定位19, char由表决定
...
#define MAXCOLLEN     100         // 表字段的最大长度
char strcolvalue[MAXCOLCOUNT][MAXCOLLEN+1];  // 存放从xml每一行解析出来的值
sqlstatement stmtins, stmtupt;    // 插入和更新表的sqlstatment
...
bool _xmltodb()
{
  ...
  while (true)
  {
    ...
    while (true)
    {
      ...
      if (iret == 3)
      {
        logfile.WriteEx("failed, xml文件打开失败\n");
        return false;
      }
...
void crtsql()
{
  ...
  // 生成插入表的SQL语句。insert into 表名(%s) valuse(%s)
  char strinsertp1[3001];  // insert语句字段列表
  ...
  for (int i = 0; i < TABCOLS.m_vallcols.size(); i++)
  {
    ...
    // 拼接strinsert2, 需区分date字段和非date字段
    char strtemp[101];
