...
bool Append(struct st_fileinfo* vfileinfo2)
{
  ...
  File.Fprintf("<filename>%s</filename><mtime>%s</mtime>/n", vfileinfo2->filename, vfileinfo2->mtime);
  return true;
}
...
bool CompVector()
{
  ...
  for(i = 0; i < vfileinfo2.size(); i++)
  {
    ...
    {
      if ((strcmp(vfileinfo2[i].filename, vfileinfo1[j].filename) == 0) && (strcmp(vfileinfo2[i].mtime, vfileinfo2[j].mtime) == 0))
      {
...
bool WriteToOKFile()
{
  ...
  {
    File.Fprintf("<filename>%s</filename><mtime>%s</mtime>", vfileinfo3[i].filename, vfileinfo3[i].mtime);
  }
  return true;
}
...
void _help()
{
  ...
  printf("<checktime>true</checktime> 是否需要检查服务端文件的时间，true-需要，false-不需要，此参数只有当ptype=1时才有效，缺省为false。\n");
