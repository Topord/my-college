...
int main(int argc,char *argv[])
{
  ...
  // 如果starg.remotecols或starg.localcols为空，就用localtname表的全部列来填充
  if ((strlen(starg.localcols) == 0) || (strlen(starg.remotecols) == 0))
  {
    CTABCOLS TABCOLS;
...
bool _syncupdate()
{
  ...
  if (starg.synctype == 1)
  {
    ...
    if (stmtins.execute() != 0)
    {
        ...
        connloc.rollback();   // 执行失败就回滚事务
        return false;
    }

    logfile.WriteEx("%d rows in %.2fsecond.ok\n", stmtins.m_cda.rpc, Timer.Elapsed());
  ...
  // 拼接SQL
  char bindstr[2001];    // 绑定同步的SQL语句参数的字符串
  char strtemp[11];
  ...
  bindstr[strlen(bindstr) - 1] = 0; // 最后一个，是多余的
...
void _help(char *argv[])
{
  printf("Using:/project/tools2/bin/syncupdate logfilename xmlbuffer\n\n");

  printf("Sample:/project/tools2/bin/procctl 10 /project/tools2/bin/syncupdate /log/idc/syncupdate_ZHOBTCODE2.log \"<localconnstr>192.168.80.129,root,123456Aa.,mysql,3306</localconnstr><charset>utf8</charset><fedtname>LK_ZHOBTCODE1</fedtname><localtname>T_ZHOBTCODE2</localtname><remotecols>obtid,cityname,provname,lat,lon,height/10,upttime,keyid</remotecols><localcols>stid,cityname,provname,lat,lon,altitude,upttime,keyid</localcols><synctype>1</synctype><timeout>50</timeout><pname>syncupdate_ZHOBTCODE2</pname>\"\n\n");

  // 因为测试的需要，xmltodb程序每次会删除LK_ZHOBTCODE1中的数据，全部的记录重新入库，keyid会变。
  // 所以以下脚本不能用keyid，要用obtid，用keyid会出问题，可以试试。
  printf("       /project/tools2/bin/procctl 10 /project/tools2/bin/syncupdate /log/idc/syncupdate_ZHOBTCODE3.log \"<localconnstr>192.168.80.129,root,123456Aa.,mysql,3306</localconnstr><charset>utf8</charset><fedtname>LK_ZHOBTCODE1</fedtname><localtname>T_ZHOBTCODE3</localtname><remotecols>obtid,cityname,provname,lat,lon,height/10,upttime,keyid</remotecols><localcols>stid,cityname,provname,lat,lon,altitude,upttime,keyid</localcols><where>where obtid like '54%%%%'</where><synctype>2</synctype><remoteconnstr>192.168.80.129,root,123456Aa.,mysql,3306</remoteconnstr><remotetname>T_ZHOBTCODE1</remotetname><remotekeycol>obtid</remotekeycol><localkeycol>stid</localkeycol><maxcount>10</maxcount><timeout>50</timeout><pname>syncupdate_ZHOBTCODE3</pname>\"\n\n");

  printf("       /project/tools2/bin/procctl 10 /project/tools2/bin/syncupdate /log/idc/syncupdate_ZHOBTMIND2.log \"<localconnstr>192.168.80.129,root,123456Aa.,mysql,3306</localconnstr><charset>utf8</charset><fedtname>LK_ZHOBTMIND1</fedtname><localtname>T_ZHOBTMIND2</localtname><remotecols>obtid,ddatetime,t,p,u,wd,wf,r,vis,upttime,keyid</remotecols><localcols>stid,ddatetime,t,p,u,wd,wf,r,vis,upttime,recid</localcols><where>where ddatetime>timestampadd(minute,-120,now())</where><synctype>2</synctype><synctype>2</synctype><remoteconnstr>192.168.80.129,root,123456Aa.,mysql,3306</remoteconnstr><remotetname>T_ZHOBTMIND1</remotetname><remotekeycol>keyid</remotekeycol><localkeycol>recid</localkeycol><maxcount>300</maxcount><timeout>50</timeout><pname>syncupdate_ZHOBTMIND2</pname>\"\n\n");
...
)ENGINE=FEDERATED CONNECTION='mysql://root:123456Aa.@192.168.80.129:3306/mysql/T_ZHOBTCODE1';
...
)ENGINE=FEDERATED CONNECTION='mysql://root:123456Aa.@192.168.80.129:3306/mysql/T_ZHOBTMIND1';
*/
