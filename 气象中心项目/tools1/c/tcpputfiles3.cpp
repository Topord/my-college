/*
 * 程序名：tcpputfiles，采用tcp协议，实现文件上客户端, 只包含ptype = 1但不能删除文件, 需编写编译指令
 * 作者：topord
*/
#include "_public.h"
...
bool _tcpputfiles()
{
  ...
  while (true)
  {
    ...
    {
      logfile.Write("_tcpputfiles() TcpClient.Read() failed\n"); return false; 
    } 
...
void _help()
{
  ...
  printf("Sample:/project/tools2/bin/procctl 20 /project/tools2/bin/tcpputfiles /log/idc/tcpputfiles_surfdata.log \"<ip>127.0.0.1</ip><port>5005</port><ptype>1</ptype><clientpath>/tmp/tcp/surfdata1</clientpath><clientpathbak>/tmp/tcp/surfdata1bak</clientpathbak><andchild>true</andchild><matchname>*.XML,*.CSV</matchname><srvpath>/tmp/tcp/surfdata2</srvpath><timetvl>10</timetvl><timeout>50</timeout><pname>tcpputfiles_surfdata</pname>\"\n");
