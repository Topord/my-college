/*
 * 程序名：tcpputfiles，采用tcp协议，实现文件上客户端. 可以异步通讯，并写入了进程的心跳信息。需独立编写编译指令
 * 作者：topord
*/
#include "_public.h"
...
bool _tcpputfiles();                // 文件上传的主函数，执行一次上传文件的操作
void EXIT(int sig);                 // 退出函数
bool SendFile(const int sockfd, const char* filename, const int size); // 上传文件给对端
...
bool _tcpputfiles()
{
  ...
  int delayed = 0;   // 为未收到对端确认报文的文件数量
  int buflen = 0;    // 接收报文的长度 
  bcontinue = false;
  ...
  {
    bcontinue = true;
    logfile.Write("_tcpputfiles() OpenDir() failed\n"); return false;
  }
  while (true)
  {
...
void Ackmessage(const char* strrecvbuffer)
{
  ...
  if (starg.ptype == 1)
  {
    logfile.Write("delete file %s ...", filename);
...
{
  printf("\n");
  printf("Using:/project/tools2/bin/tcpputfiles logfilename xmlbuffer\n\n");

  printf("Sample:/project/tools2/bin/procctl 20 /project/tools2/bin/tcpputfiles /log/idc/tcpputfiles_surfdata.log \"<ip>192.168.80.129</ip><port>5005</port><ptype>1</ptype><clientpath>/tmp/tcp/surfdata1</clientpath><clientpathbak>/tmp/tcp/surfdata1bak</clientpathbak><andchild>true</andchild><matchname>*.XML,*.CSV</matchname><srvpath>/tmp/tcp/surfdata2</srvpath><timetvl>10</timetvl><timeout>50</timeout><pname>tcpputfiles_surfdata</pname>\"\n");
  printf("       /project/tools1/bin/procctl 20 /project/tools1/bin/tcpputfiles /log/idc/tcpputfiles_surfdata.log \"<ip>127.0.0.1</ip><port>5005</port><ptype>1</ptype><clientpath>/tmp/tcp/surfdata1</clientpath><clientpathbak>/tmp/tcp/surfdata1bak</clientpathbak><andchild>true</andchild><matchname>*.XML,*.CSV</matchname><srvpath>/tmp/tcp/surfdata2</srvpath><timetvl>10</timetvl><timeout>50</timeout><pname>tcpputfiles_surfdata</pname>\"\n\n\n");
