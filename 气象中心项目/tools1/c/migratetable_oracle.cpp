...
int main(int argc,char *argv[])
{
  ...
  if (logfile.Open(argv[1],"2")==false)
  {
    printf("打开日志文件失败（%s）。\n",argv[1]); return -1;
  }
...
void _help(char *argv[])
{
  printf("Using:/project/tools2/bin/migratetable_oracle logfilename xmlbuffer\n\n");

  printf("Sample:/project/tools2/bin/procctl 3600 /project/tools2/bin/migratetable_oracle /log/idc/migratetable_oracle_ZHOBTMIND1.log \"<connstr>qxidc/qxidcpwd@snorcl11g_132</connstr><srctname>T_ZHOBTMIND1</srctname><dsttname>T_ZHOBTMIND_HIS</dsttname><keycol>keyid</keycol><where>where ddatetime<sysdate-0.03</where><starttime>01,02,03,04,05,13</starttime><maxcount>300</maxcount><timeout>120</timeout><pname>migratetable_oracle_ZHOBTMIND1</pname>\"\n\n");
