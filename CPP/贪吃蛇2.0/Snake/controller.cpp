...
int Controller::PlayGame()
{
    ...
    Food* cfood = new Food();
    Terrain* cterrain = new Terrain();
    SetColor(6);
    csnake->InitSnake();
    srand((unsigned)time(NULL));//设置随机数种子，如果没有 食物的出现位置将会固定
    cfood->DrawFood(*csnake, *cterrain);

    //画出不同的地形
    cterrain->TerrInform(choice);
    cterrain->PrintTerrain();
    /*游戏循环*/
    while (csnake->HitItself() && csnake->HitTer(*cterrain)) //判断是否撞墙或撞到自身，即是否还有生命
    {
        /*调出选择菜单*/
        ...
        if (csnake->GetFood(*cfood)) //吃到食物
        {
            ...
            RewriteScore();//重新绘制分数
            cfood->DrawFood(*csnake, *cterrain);//绘制新食物
        }
        else
        {
...
{
    score += key * tmp;//所得分数根据游戏难度及传人的参数tmp确定
}
//重绘分数
...
void Controller::Game()
{
    ...
    while (true)//游戏可视为一个死循环，直到退出游戏时循环结束
    {
        ...
        if (tmp == 1) //返回值为1时重新开始游戏
        {
            ...
            // 打印snake
            SetColor(11);
            StartInterface* start1 = new StartInterface();
            start1->MidSnake();
            delete start1;
